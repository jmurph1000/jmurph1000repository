...
const sessionStore = new SQLiteStore({ db: 'sessions.sqlite', dir: path.join(__dirname, '..', '.data') }) as unknown as session.Store;
app.use(
  session({
    store: sessionStore,
...
app.get('/', (_req, res) => {
  res.render('index', { user: (res.locals as any).user || null });
});
...
app.use(async (req, res, next) => {
  ...
  (res.locals as any).isTreasury = !!user?.roles.some((r) => r.role === RoleType.TREASURY);
  next();
});
...
app.post('/admin/access/role', requireRole([RoleType.TREASURY, RoleType.ADMIN]), async (req, res) => {
  const { userId, role, enable } = req.body as { userId: string; role: RoleType; enable: string };
  if (enable === 'true') {
  ...
  if (user) await sendEmail(user.email, 'Access Updated', `Your role ${role} has been ${enable === 'true' ? 'granted' : 'revoked'}.`);
  res.redirect('/admin/access');
});

app.post('/admin/access/capability', requireRole([RoleType.TREASURY, RoleType.ADMIN]), async (req, res) => {
  const { userId, category, type } = req.body as { userId: string; category: PaymentCategory; type: 'INITIATE'|'APPROVE'|'BOTH' };
  ...
  if (capUser) await sendEmail(capUser.email, 'Capability Updated', `Your capability for ${category} is now ${type}.`);
  res.redirect('/admin/access');
});
...
app.post('/requests/:id/approve', requireAuth, async (req, res) => {
  ...
  if (payment.createdById === userId) return res.status(403).send('Cannot approve your own request');
  ...
  if (!allowed || (allowed.type !== 'APPROVE' && allowed.type !== 'BOTH')) return res.status(403).send('No approve permission');
...
app.post('/requests/:id/reject', requireAuth, async (req, res) => {
  ...
  if (payment.createdById === userId) return res.status(403).send('Cannot reject your own request');
  const allowed = await prisma.capability.findFirst({ where: { userId, category: payment.category } });
  if (!allowed || (allowed.type !== 'APPROVE' && allowed.type !== 'BOTH')) return res.status(403).send('No approve permission');
...
app.post('/requests/:id/treasury-approve', requireRole([RoleType.TREASURY]), async (req, res) => {
  ...
  if (p?.createdBy?.email) await sendEmail(p.createdBy.email, 'Payment Approved by Treasury', `Payment ${id} has been approved by Treasury.`);
...
app.post('/requests/:id/treasury-reject', requireRole([RoleType.TREASURY]), async (req, res) => {
  ...
  const p = await prisma.payment.findUnique({ where: { id }, include: { createdBy: { select: { email: true } } } });
  if (p?.createdBy?.email) await sendEmail(p.createdBy.email, 'Payment Rejected by Treasury', `Payment ${id} has been rejected by Treasury.`);
  res.redirect(`/requests/${id}`);
});
...
app.post('/requests/new/funding', requireAuth, async (req, res) => {
  const userId = req.session.userId!;
  const { fundingType, amount, reason } = req.body as { fundingType: FundingType; amount: string; reason: string };

  const parsedAmount = Number(amount);
  if (!Number.isFinite(parsedAmount) || parsedAmount <= 0) return res.status(400).send('Invalid amount');
  ...
  if (!canInit || (canInit.type !== 'INITIATE' && canInit.type !== 'BOTH')) return res.status(403).send('No initiate permission');
  ...
  const denominator: number = median === 0 ? 1 : (median as number);
  ...
  const approverEmailsFunding = (
    await prisma.capability.findMany({ where: { category: PaymentCategory.FUNDING }, include: { user: true } })
  )
...
app.post('/requests/new/ap', requireAuth, async (req, res) => {
  ...
  const approverEmailsAP = (
    await prisma.capability.findMany({ where: { category: PaymentCategory.AP_FUNDING }, include: { user: true } })
  )
    .filter((c) => c.type === 'APPROVE' || c.type === 'BOTH')
    .map((c) => c.user.email);
...
app.post('/requests/new/specific', requireAuth, async (req, res) => {
  ...
  const approverEmailsSpec = (
    await prisma.capability.findMany({ where: { category: PaymentCategory.SPECIFIC }, include: { user: true } })
  )
    .filter((c) => c.type === 'APPROVE' || c.type === 'BOTH')
    .map((c) => c.user.email);
